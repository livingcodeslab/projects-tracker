...
responses that had a very similar shape, represented below:
...
the response represented below:
...
in the negative range, we get results of the following shapes
...
meanwhile, with V_{DS} in the positive range, we get the following shapes:
