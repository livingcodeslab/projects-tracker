...
#+CAPTION: Chip Characteristics with distilled-water after switching to side A of the cartridge, and drain-source channel 3 (source 3).
...
Next, we'll try with the drop-type reservoir, for good measure. We begin by resetting the device.
...
Next, we disassemble the cartridge and replace the flowcell-type reservoir with the drop-type reservoir. We can then drop some distilled water (~60µL) into the reservoir and run the sweep.
...
#+begin_src gnuplot :tangle ./20260526-004-water-readings.gp
  load "./20260220-plotting-styles.gp"

  set output "./static/20260526-004-water-readings-positive.svg"
  ...
  set datafile separator ","
  plot \
       "./static/20260526-004-water-readings_positive.csv" \
       using "measured_gate_voltage":"drain_current" \
       title "Distilled Water Environment Characteristics" \
       with lines
#+end_src
...
#+CAPTION: Chip Characteristics with distilled-water on side A of the cartridge, using drop-type reservoir, and drain-source channel 3 (source 3).
#+NAME: 20260526-004-water-readings-positive
[[file:static/20260526-004-water-readings-positive.svg]]

Aaargh. This is not good.

We should test with the drop-type reservoir for more of the chip's drain-source channels.

** Conclusions

It looks like the over-voltage broke the powered side of the chip completely.

The other side that was not powered during the over-voltage seems to be mostly okay, other than the fact that the dirac point is not within the acceptable range.

Running the sweep with the drop-type reservoir gives completeley unexpected results and the currents are completely out of range.
